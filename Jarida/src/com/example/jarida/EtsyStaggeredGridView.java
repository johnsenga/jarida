 ...
 * <p/>
 * Builds multiple columns on top of {@link EtsyExtendableListView}
 * <p/>
 * Partly inspired by - https://github.com/huewu/PinterestLikeAdapterView
 */
public class EtsyStaggeredGridView extends EtsyExtendableListView {
    ...
    public EtsyStaggeredGridView(final Context context) {
        this(context, null);
    }

    public EtsyStaggeredGridView(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public EtsyStaggeredGridView(final Context context, final AttributeSet attrs, final int defStyle) {
        super(context, attrs, defStyle);

        if (attrs != null) {
            // get the number of columns in portrait and landscape
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.EtsyStaggeredGridView, defStyle, 0);

            mColumnCount = typedArray.getInteger(
                    R.styleable.EtsyStaggeredGridView_column_count, 0);
            ...
            if (mColumnCount > 0) {
                ...
                mColumnCountPortrait = typedArray.getInteger(
                        R.styleable.EtsyStaggeredGridView_column_count_portrait,
                        DEFAULT_COLUMNS_PORTRAIT);
                mColumnCountLandscape = typedArray.getInteger(
                        R.styleable.EtsyStaggeredGridView_column_count_landscape,
                        DEFAULT_COLUMNS_LANDSCAPE);
            }

            mItemMargin = typedArray.getDimensionPixelSize(
                    R.styleable.EtsyStaggeredGridView_item_margin, 0);
            mGridPaddingLeft = typedArray.getDimensionPixelSize(
                    R.styleable.EtsyStaggeredGridView_grid_paddingLeft, 0);
            mGridPaddingRight = typedArray.getDimensionPixelSize(
                    R.styleable.EtsyStaggeredGridView_grid_paddingRight, 0);
            mGridPaddingTop = typedArray.getDimensionPixelSize(
                    R.styleable.EtsyStaggeredGridView_grid_paddingTop, 0);
            mGridPaddingBottom = typedArray.getDimensionPixelSize(
                    R.styleable.EtsyStaggeredGridView_grid_paddingBottom, 0);
